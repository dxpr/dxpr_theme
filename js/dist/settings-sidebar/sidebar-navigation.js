...
function initializeSidebarNavigation() {
  ...
  const addNavigationHandler = () => {
    ...
    menuItems.forEach(menuItem => {
      const menuTitle = menuItem.querySelector('.vertical-tabs__menu-item-title');
      if (menuTitle) {
        const titleText = menuTitle.textContent.trim();
        
        if (navigationMap.hasOwnProperty(titleText)) {
  ...
  const scrollToSection = (sectionId) => {
    ...
    if (sectionId === 'top') {
      ...
      // Find the section by ID within the styleguide
      targetElement = styleguideContainer.querySelector(`#${sectionId}`);
    }
