...
Drupal.behaviors.fullScreenSearch = {
    attach: function(context, settings) {
        ...
        $(document).keydown(function(event) {
            event.which === escapeCode && !$searchForm.hasClass("element-invisible") && clearSearchForm()
        })
    }
}
...
$(document).ready(function () {
  ...
  if ($('.glazed-header--sticky').length > 0 && $(window).width() > navBreak) {
    var headerHeight = Drupal.settings.glazed.headerHeight;
    var headerScroll = Drupal.settings.glazed.headerOffset;
    var scroll = 0;
...
function markMenu() {
  var markMenuItems = Drupal.settings.glazed.markMenuWithChildren;
  var childrenSymbolClass = Drupal.settings.glazed.childrenSymbolClass;
