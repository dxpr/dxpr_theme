...
(function($, Drupal, window, document, undefined) {
    ...
    attach(context, settings) {
      ...
      $(document).ready(() => {
        if (Drupal.settings.glazed.breadcrumbsSeparator) {
          const { breadcrumbsSeparator } = Drupal.settings.glazed;
      ...
      $(".node-event .field-event-location", context)
        ...
        .wrapInner(
          `<a href="https://maps.google.com/?q=${$(
            ".node-event .field-event-location"
          ).text()}">`
        );
    }
  };
  ...
  $(document).ready(() => {
    ...
    if (
      ...
      const { headerHeight } = Drupal.settings.glazed;
      const headerScroll = Drupal.settings.glazed.headerOffset;
      let scroll = 0;
    ...
    if (
      ...
      if (
        ...
        if (
          ...
          if (Drupal.settings.glazed.headerOffset) {
  ...
  $(document).ready(() => {
    const headerHeight = Drupal.settings.glazed.headerMobileHeight;
    const headerFixed = Drupal.settings.glazed.headerMobileFixed;
